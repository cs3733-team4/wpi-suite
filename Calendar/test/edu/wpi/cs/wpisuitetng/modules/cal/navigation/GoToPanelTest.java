...
public class GoToPanelTest
{
	...
	public void setup()
	{
		dummyPanel = MainPanel.getInstance();

		now = new DateTime(2000, 6, 1, 0, 0);
		then = new DateTime(2000, 7, 1, 0, 0);
		before = new DateTime(1900, 7, 1, 0, 0);
		after = new DateTime(2100, 7, 1, 0, 0);
	}
	...
	public void testGotoErrorEarly() throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException
	{
		...
		assertEquals("If the input year is below 1900, the gotoDate box function will cause an error to display on the gotoErrorText box",
				"* Year out of range (1900-2100)", ((JLabel) f.get(GTP)).getText());

		// The error text will remain in the GoToPanel even if a correct date is
		// given after the error; it will simply stop displaying the error
		// message until a new error occurs
		// It is therefore necessary to create a new GoToPanel for each test, so
		// the error text will reset back to "  "
	...
	public void testGotoErrorLate() throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException
	{
		...
		assertEquals("If the input year is above 2100, the gotoDate box function will cause an error to display on the gotoErrorText box",
				"* Year out of range (1900-2100)", ((JLabel) f.get(GTP)).getText());
	...
	public void testGotoErrorFormat() throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException
	{
		...
		assertEquals("If the input is in the incorrect format, the gotoDate box function will cause an error to display on the gotoErrorText box",
				"* Use format: mm/dd/yyyy", ((JLabel) f.get(GTP)).getText());
	...
	public void testGotoErrorInput() throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException
	{
		...
		assertEquals("If the input isn't an acceptable mm/dd/yyyy , the gotoDate box function will cause an error to display on the gotoErrorText box",
				"* Use format: mm/dd/yyyy", ((JLabel) f.get(GTP)).getText());
		...
		assertEquals("We do not accept the English version of dd/m/yyyy, regardless of how amazingly logical it is", "* Use format: mm/dd/yyyy",
				((JLabel) f.get(GTP)).getText());
	}
